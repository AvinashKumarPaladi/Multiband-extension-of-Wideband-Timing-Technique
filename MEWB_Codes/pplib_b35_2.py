...
def write_TOAs(TOAs, inf_is_zero=True, SNR_cutoff=0.0, outfile=None,
    ...
    for toa in toas:
        if toa.frequency == np.inf and inf_is_zero:
            toa_string = "%s %.8f %d" % (toa.archive_band3+"_&_"+toa.archive_band5, 0.0,
                                         toa.MJD.intday()) + ("%.15f   %.3f  %s" % (toa.MJD.fracday(),
                                                                                    toa.TOA_error, toa.telescope_code_band3))[
                                                             1:]
        else:
            toa_string = "%s %.8f %d" % (toa.archive_band3+"_&_"+toa.archive_band5, toa.frequency,
                                         toa.MJD.intday()) + ("%.15f   %.3f  %s" % (toa.MJD.fracday(),
                                                                                    toa.TOA_error, toa.telescope_code_band3))[
                                                             1:]
